...
def start_receivers(environment_repository : EnvironmentRepository, configurations):
    threads = []

    for ReceiverClass in RECEIVER_REGISTRY.values():
        provider = ReceiverClass.provider
        environments = environment_repository.get_environments_by_provider(provider)

        logger = LoggingUtils(provider, configurations)

        ReceiverClass.pre_start()

        for current_environment, environment_specs in environments.items():
            receiver = ReceiverClass(current_environment, environment_specs, configurations, logger)
            receiver.start()
...
def launch_receivers(environment_repository : EnvironmentRepository, configurations, logger):

    threads = start_receivers(environment_repository, configurations)
